...
// filter to increase API timeout limit to 15 seconds
function custom_http_request_timeout( ) {
    return 15;
}
add_filter( 'http_request_timeout', 'custom_http_request_timeout' );

function team_rosters_shortcode($atts) {
    // Extract shortcode attributes, allow for passing city, program, and gender
    $atts = shortcode_atts(array(
        'city' => 'Anaheim',  // Default is 'Anaheim'
        'program' => 'Select',  // Default is 'Select'
        'gender' => 'Boys',  // Default is 'Boys'
    ), $atts, 'team_rosters');

    // Construct the API URL based on the passed parameters
    $api_url = "https://script.google.com/macros/s/AKfycbx2eGgrD6GolN9PWIhLk7VFtoeWcAGCs6f8OMKlCDjF-hC4prtM12RcNDsDTtrBnfsRpQ/exec?programName=" .
               urlencode($atts['city']) . "%20" .
               urlencode($atts['program']) . "%20-%20" .
               urlencode($atts['gender']);
    
    // Perform the GET request using WordPress' HTTP API
    ...
    if (isset($data['status']) && $data['status'] === "error") {
        return "<p>Could not load roster information.</p>";
    }
    ...
    <h3 class='wp-block-heading xlarge-margin-top' id='h-orange-county-ogp-anaheim-club-team-rosters'>
        <?php echo htmlspecialchars($data['program']) . " Rosters" ?>
    </h3>
    <?php if (!empty($data['data'])): ?>
        <?php foreach ($data['data'] as $team): ?>
        <div class="wp-block-group is-layout-constrained">
            <div class="wp-block-group__inner-container">
                <div class="wp-block-group is-layout-constrained">
                    <div class="wp-block-group__inner-container">
                        <div class="wp-block-group is-layout-flow">
                            <div class="wp-block-group__inner-container">
                                <h4 class="wp-block-heading header-bar">
                                    <?php echo htmlspecialchars($team['clubDetails']['team'] ?? 'Team Name Unavailable'); ?>
                                </h4>
                            </div>
                        </div>
                    </div>
                ...
                <p>
                    Coach: <?php echo htmlspecialchars($team['clubDetails']['coach'] ?? 'Coach Unavailable'); ?>
                </p>
                <p>
                    Practice: <?php echo htmlspecialchars($team['clubDetails']['practice'] ?? 'Practice Info Unavailable'); ?>
                </p>
                <div class="wp-block-group is-layout-constrained">
                    <div class="wp-block-group__inner-container">
                        <div class="wp-block-group is-layout-flow">
                            <div class="wp-block-group__inner-container">
                                <div class="wp-block-columns is-layout-flex wp-container-11">
                                    <div class="wp-block-column is-layout-flow" style="flex-basis:25%">
                                        <h5 class="wp-block-heading header-bar" id="h-rosters-7">Rosters</h5>
                                    ...
                                    <div class="wp-block-column is-layout-flow" style="flex-basis:75%">
                                        <h5 class="wp-block-heading header-bar" id="h-schedule-7">Schedule</h5>
                                        <figure class="wp-block-table">
                                            <!-- Events Table Loop -->
                                            <table>
                                                <thead>
                                                ...
                                                <tbody>
                                                    <?php if (!empty($team['events'])): ?>
                                                        ...
                                                        <?php endforeach; ?>
                                                    <?php else: ?>
                                                        <tr><td colspan="3">No events listed.</td></tr>
                                                    <?php endif; ?>
                                                </tbody>
                                            </table>
                                        </figure>
...
// Register the shortcode [anaheim_team_roster city="Anaheim" program="Select" gender="Boys"]
add_shortcode('team_rosters', 'team_rosters_shortcode');
